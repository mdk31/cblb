...
makeK_linear <- function(allx, useasbases=NULL, b=NULL, linkernel = FALSE, scale = TRUE){
  N <- nrow(allx)
  
  # If no "useasbases" given, assume all observations are to be used.
  if (is.null(useasbases)) {
    useasbases <- rep(1, N)
  }
  
  # Default b is set to 2 * ncol to match kbal for now
  if (is.null(b)) {
    b <- 2 * ncol(allx)
  }
  ...
  bases <- allx[useasbases == 1, , drop = FALSE]
...
osqp_kernel_sbw <- function(X,A,Y,
  ...
  Xt <- X_[A==1,]; Xc <- X_[A==0,]
  Yt <- Y[A==1]; Yc <- Y[A==0]
  P.mat <- as(Matrix::.symDiagonal(n=n0, x=1.), "dgCMatrix")
  q.vec <- rep(-1./n0,n0)

  A.mat <- Matrix::Matrix(rbind(rep(1.,n0),
                        P.mat,
                        t(Xc)), sparse = TRUE)
...
osqp_kernel_sbwN0 <- function(X,A,Y,
                            delta.v=0.005, 
                            X_=NULL,
                            osqp.setting=NULL,
                            basis="kernel", kernel.approximation=TRUE,
  ...
  A.mat <- rbind(
    rep(1.0, n0),    # sum(w) row
    diag(1.0, n0),   # w_i >= 0
    t(Xc)            # sum_{i=1}^{n0} w_i * X_{c,i,k}
  )
  ...
  for (j in 1:length(delta.v)) {
    ...
    # u.vec <- c(1., rep(1.,n0),
    #            colMeans(Xt) + delta.v[j] * rep(1,nX))
    # Suppose sum(w) = n0, then reweighted mean of Xc ~ colMeans(Xt).
    # => sum_{i} w_i * X_{c,i} ~ n0 * colMeans(Xt).
    
    l.vec <- c(
      n0,                     # sum(w) >= n0 => sum(w)=n0
      rep(0.0, n0),           # w_i >= 0
      n0*(colMeans(Xt) - delta.v)    # scale by n0
    )
